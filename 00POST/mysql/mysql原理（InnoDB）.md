...
2. 该日志文件由两部分组成:
	...
	- redolog说到底就是保证数据的一致性，**redo log刷盘是顺序IO**，磁盘上会划出一个固定大小的空间，这个空间可以看作是**循环复用**的环形空间。而**脏页刷盘是随机IO**，比如一条sql可能涉及多个数据页，但修改的数据可能是离散分布的，此时刷盘就涉及到一些额外的操作（比如磁盘的磁头寻道等），这肯定是不如顺序IO的。所以脏页刷盘慢，redo log刷盘快，时间越久意味发生数据库故障的可能性越高，为了保证事务下数据一致性，会确保redo log日志先刷完盘（此时就可以**认为**事务提交成功），再刷脏页，这时就算脏页刷盘发生故障，也可以用redo log重做。
### 回滚日志（Undo Log）
1. 记录了**事务发生之前**的数据状态(不包括select).如果修改数据时出现异常,可以使用undo log来实现回滚操作(**保持原子性**)
...
![[Pasted image 20240216204730.png]]
## 执行引擎（InnoDB）
